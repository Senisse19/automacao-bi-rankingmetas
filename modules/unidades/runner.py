    ...
    def process_reports(
        ...
        try:
            ...
            if daily:
                ...
                if not has_daily_items:
                    ...
                    if not generate_only:
                        ...
                        self._send_image_to_group(
                            "diretoria",
                            daily_path,
                            base_caption,
                            custom_recipients=recipients,
                            template_content=template_content,
                            data_ref_str=data_display,
                        )
                    else:
            ...
            if should_run_weekly:
                ...
                logger.info(
                    f"   [Processando] Relatório Semanal (Semana Anterior Completa): {start_weekly} a {data_ref_weekly}"
                )
                ...
                if not has_weekly_items:
                    logger.info(
                        f"   [SKIP] Nenhum dado encontrado para o relatório semanal ({start_weekly} a {data_ref_weekly})."
                    )
                else:
                    # --- LIVE LINK ---
                    ...
                    weekly_path = os.path.join(
                        IMAGES_DIR,
                        f"Relatório de Unidades Semanal {start_weekly} a {data_ref_weekly}.pdf",
                    )
