...
class InaRenderer(BaseRenderer):
    """
    Renderizador para o Painel INA (Inadimplência).
    """

    def generate_image(self, kpis, top10, output_path="ina_report.png", area_name="GERAL"):
        """
        Gera a imagem do relatório diário do INA.
        """
        ...
        # Footer precisa de espaço extra para não sobrepor o card
        ...
        # Ajusta subtítulo com a área
        sub_text = f"Posição: {now_str} | Área: {area_name}"
        ...
        # Vamos fazer 2 linhas de 2 cards para ficar legível no mobile.
    ...
    def _draw_kpi_cards(self, draw, start_y, kpis):
        ...
        # Definição dos dados para exibir
        # Formato: (Titulo, Valor, CorDestaque?)
        cards_data = [
            ("VENCENDO HOJE", kpis.get("VencendoHoje"), True),  # Destaque
            ("INADIMPLÊNCIA TOTAL", kpis.get("Total"), False),
            ("MÉDIA ATRASO (DIAS)", str(kpis.get("MediaAtraso", "-")), False),
            ("CONCILIAÇÃO (D+2)", kpis.get("Conciliacao"), False),
        ]
    ...
    def _draw_top10_table(self, draw, start_y, top10):
        ...
        # 2. Título da Seção (Dentro do Card ou Fora? User disse "grande card com colunas", entao titulo pode ser dentro)
        ...
        # Ajuste de Colunas com posições fixas e alinhamento consistente
        ...
        for i, item in enumerate(top10):
            ...
            # Zebra Striping - TODAS as linhas têm a mesma altura (row_h)
            if i % 2 == 1:
                bg_zebra = (28, 28, 28)
                row_top = row_y
